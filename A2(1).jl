...
student = (name="Aditya Vaidya", email="avaidya1@ualberta.ca", ccid="avaidya1", idnumber=1769969)
...
function epoch!(reg::SimpleStochasticRegressor, 
	...
	len_of_X = length(X)
	parameter = reg.w
	nums_arr = rand(1:len_of_X, len_of_X)
	for i in nums_arr
		parameter = parameter - ((reg.η * ((X[i]*parameter) - Y[i]))*(X[i]))
		end
	reg.w = parameter
	#### END SOLUTION
end
...
function epoch!(reg::MiniBatchRegressor,
	...
	#### BEGIN SOLUTION
	len_of_X = length(X)
	nums_arr = rand(1:len_of_X, reg.b)
	for j in 1:trunc(Int, ceil(len_of_X / reg.b))
		sum_of_all = 0
		for i in nums_arr
			sum_of_all = sum_of_all + (((reg.w * X[i]) - Y[i]) * X[i])
		end
		g = sum_of_all / (reg.b)
		reg.w = reg.w - reg.η * g
	end
...
let
	if __run_qb_experiments
		...
		#mean(results_qb["Mean"].test_error)
		
		std(results_qb["Mean"].test_error)
	end
end
...
let
	if __run_q3_experiments
		...
		a = mean(results["Mean"].test_error)
		#b = mean(results["Batch"].test_error)
		c = std(results["Mean"].test_error)
		#d = std(results["Batch"].test_error)
		print(a,c)
	end
end
